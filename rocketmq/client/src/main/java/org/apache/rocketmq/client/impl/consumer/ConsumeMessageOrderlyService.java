...
public class ConsumeMessageOrderlyService implements ConsumeMessageService {
    ...
    public ConsumeMessageOrderlyService(DefaultMQPushConsumerImpl defaultMQPushConsumerImpl,
        ...
        if (this.defaultMQPushConsumer.getSharedThreadPoolExecutor() == null) {
            ...
            this.consumeExecutor = this.defaultMQPushConsumer.getSharedThreadPoolExecutor();
        }

        if (this.defaultMQPushConsumer.getSharedScheduledExecutorService() == null) {
            this.scheduledExecutorService = Executors.newSingleThreadScheduledExecutor(
                    new ThreadFactoryImpl("ConsumeMessageScheduledThread_" + consumerGroup + "_"));
        } else {
            this.scheduledExecutorService = this.defaultMQPushConsumer.getSharedScheduledExecutorService();
        }
    }

    public void start() {
        if (MessageModel.CLUSTERING.equals(ConsumeMessageOrderlyService.this.defaultMQPushConsumerImpl.messageModel())) {
            scheduledFuture = this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
    ...
    public boolean processConsumeResult(
        ...
        if (context.isAutoCommit()) {
            ...
            switch (status) {
                case SUCCESS:
                    consumeRequest.getProcessQueue().commit(); //add by jht.
                    this.getConsumerStatsManager().incConsumeOKTPS(consumerGroup, consumeRequest.getMessageQueue().getTopic(), msgs.size());
                    break;
                case COMMIT:
                    commitOffset = consumeRequest.getProcessQueue().commit();
                ...
                case SUSPEND_CURRENT_QUEUE_A_MOMENT:
                    ...
                    break;
                case SUSPEND_CURRENT_QUEUE_A_MOMENT_FOR_BLOCK:
                    consumeRequest.getProcessQueue().makeMessageToCosumeAgain(msgs);
                    this.submitConsumeRequestLater(
                            consumeRequest.getProcessQueue(),
                            consumeRequest.getMessageQueue(),
                            this.defaultMQPushConsumer.getSuspendCurrentQueueForBlockTimeMillis());
                    continueConsume = false;
                    break;
                default:
                    break;
